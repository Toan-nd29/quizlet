...
export function LearnMode({ setId }: { setId: string }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { const data = await api<StudySet>(`/api/sets/${setId}`); setSet(data); start(data); } catch (value) { setError(value instanceof Error ? value.message : 'Không thể tải chế độ học.'); } finally { setLoading(false); } }, [setId, start]);
  useEffect(() => { void load(); }, [load]);

  const finishSession = useCallback(async () => { setFinished(true); try { await api('/api/sessions', { method: 'POST', body: JSON.stringify({ studySetId: setId, mode: 'LEARN', startedAt: startedAt.current, correctAnswers, incorrectAnswers, totalAnswers: correctAnswers + incorrectAnswers }) }); } catch { /* Metrics are best effort. */ } }, [correctAnswers, incorrectAnswers, setId]);
  ...
  return <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8"><div className="mb-5"><Link href={`/sets/${setId}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{set.title}</Link></div><ModeNav setId={setId} active="learn" /><div className="mb-5"><div className="mb-2 flex justify-between text-sm"><span className="font-medium">Tiến độ</span><span className="text-muted-foreground">{mastered} / {set.items.length}</span></div><Progress value={progressPercent} className="[&_[data-slot=progress-track]]:h-2" /></div>
    <section className="rounded-3xl border border-border bg-card p-5 shadow-[0_24px_80px_rgba(0,0,0,.2)] sm:p-8"><span className="text-xs font-bold uppercase tracking-[.12em] text-violet-300">Câu {Math.min(position + 1, queue.length)} · Chọn một đáp án</span><h1 className="mt-5 whitespace-pre-wrap text-xl font-semibold leading-relaxed sm:text-2xl">{prompt}</h1>
      ...
      {!feedback ? <div className="mt-5 text-right"><Button variant="ghost" onClick={() => void answer('', true)} disabled={submitting}>Không biết?</Button></div> : <div className={`mt-6 rounded-2xl border p-5 ${feedback.correct ? 'border-emerald-400/35 bg-emerald-400/8' : 'border-rose-400/35 bg-rose-400/8'}`}><div className="flex items-center gap-2 font-semibold">{feedback.correct ? <><CheckCircle2 className="text-emerald-300" />Chính xác!</> : <><XCircle className="text-rose-300" />Chưa chính xác</>}</div>{!feedback.correct && <p className="mt-3 text-sm"><span className="text-muted-foreground">Đáp án đúng:</span> <strong>{feedback.correctAnswer}</strong></p>}{current.explanation && <p className="mt-2 text-sm leading-6"><span className="text-muted-foreground">Giải thích:</span> {current.explanation}</p>}<div className="mt-4 flex justify-end">{feedback.correct ? <span className="text-sm text-muted-foreground">Đang chuyển sang câu tiếp theo…</span> : <Button onClick={continueNext}>Tiếp tục <kbd className="ml-2 rounded bg-white/10 px-1.5 py-0.5 text-[10px]">Enter</kbd></Button>}</div></div>}
    </section></main>;
}
