...
export function LearnMode({ setId }: { setId: string }) {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8"><div className="mb-5"><Link href={`/sets/${setId}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{set.title}</Link></div><ModeNav setId={setId} active="learn" /><div className="mb-5"><div className="mb-2 flex justify-between text-sm"><span className="font-medium">Tiến độ</span><span className="text-muted-foreground">{progressSeen} / {set.items.length}</span></div><Progress value={progressPercent} className="[&_[data-slot=progress-track]]:h-2" /></div>
